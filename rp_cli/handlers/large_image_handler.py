...
def handle_large_image_update():
    ...
    display_message("🔑 The large image field consists of a key and text. Choose which to update.\n\n🔙 Ctrl+Shift+C: Cancel edit")
    questions = [
    ...
    if subfield == 'key':
        current_key = read_current_value('key', subfield_of="large_image")
        questions = [
            inquirer.Text('key', message="Enter new key for large image", default=current_key, validate=validate_input)
        ]
        ...
        current_text = read_current_value('text', subfield_of="large_image")
        questions = [
            inquirer.Text('text', message="Enter new text for large image", default=current_text, validate=validate_input)
        ]
        answers = inquirer.prompt(questions)
        if answers is None:
            clear()
    ...
    clear()
    return display_message("Large image updated successfully 🙌🎉")